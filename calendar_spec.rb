...
describe Calendar do 
	...
	context "with valid input" do
		...
		it "should add the given event at the correct date" do
			calendar.add_event(@event.description, @date.day, @date.month, @date.year)
		...
		it "should remove an event given its id and date" do
			...
			calendar.remove_event(event_id, @date.day, @date.month, @date.year)
		...
		it "should edit the description of an event already added" do
			...
			event_id = calendar.get_events_by_date(@event.date)[0].id
			calendar.edit_event(event_id, new_desc, @date.day, @date.month, @date.year)
		...
		it "should be able to add multiple events on the same date" do
			calendar.add_event(@event.description, @date.day, @date.month, @date.year)
			calendar.add_event("Go to university", @date.day, @date.month, @date.year)
			calendar.add_event("Lunch at KFC", @date.day, @date.month, @date.year)
